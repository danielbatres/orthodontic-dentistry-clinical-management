 ...
 * @since 24/09/22
 */
public class ModifyPaciente extends Styles {
    ...
    private final ArrayList<JPanel> containerButtons = new ArrayList<>();
    
    /**
     * Creates new form ModifyPaciente
     */
    public ModifyPaciente() {
        initComponents();
        styleMyComponentBaby();
        Tools.showPanel(dataContent, ApplicationContext.modificarInformacionPaciente, 10, 10);
    }
    ...
    private void odontologiaContainerMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_odontologiaContainerMouseClicked
        ...
        if (NewContext.isTratamientoOdontologico) {
            setOdontologia(ApplicationContext.modificarOdontologiaPaciente);
        } else {
    ...
    private void ortodonciaContainerMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ortodonciaContainerMouseClicked
        ...
        if (NewContext.isTratamientoOrtodontico) {
            setOrtodoncia(ApplicationContext.modificarOrtodonciaPaciente);
        } else {
    ...
    private void saveButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_saveButtonMouseClicked
        ...
        NewContext.validarTodosLosCampos(ApplicationContext.modificarInformacionPaciente.camposTexto, ApplicationContext.modificarInformacionPaciente.advertenciasTexto, ApplicationContext.modificarInformacionPaciente.containersTexto);
        ApplicationContext.modificarInformacionPaciente.validarCombo();
        ApplicationContext.modificarInformacionPaciente.validarEdad();
        ApplicationContext.modificarInformacionPaciente.verifyEmpty();
        ApplicationContext.modificarInformacionPaciente.validarFechaNacimiento();
        
        if (NewContext.countedErrors == 0) {
            NewContext.paciente = ApplicationContext.modificarInformacionPaciente.devolverDatos();
            
            if (NewContext.paciente.isTratamientoOdontologico()) {
                NewContext.paciente.setFichaOdontologia(ApplicationContext.modificarOdontologiaPaciente.devolverFicha());
                ApplicationContext.modificarOdontologiaPaciente.verifyEmpty();
            } 

            if (NewContext.paciente.isTratamientoOrtodontico()) {
                NewContext.paciente.setFichaOrtodoncia(ApplicationContext.modificarOrtodonciaPaciente.devolverFicha());
                ApplicationContext.modificarOrtodonciaPaciente.verifyEmpty();
            }
            
            if (NewContext.emptyCounter != 0) {
                EstasSeguroEdicion estasSeguroEdicion = new EstasSeguroEdicion();
                estasSeguroEdicion.setVisible(true);
            } else {
                PacienteHelper.updatePacientData(NewContext.paciente);
            }
        }
